...
//   return (
//     <div>
//       <div className="flex justify-between items-center w-full p-3 mobile:w-40 gap-2">
...
const Bar = ({ title, type, onAddStory, buttonText }) => {
  return (
    <div>
      <div className="flex justify-between items-center w-full p-3 mobile:w-32 gap-2">
        <h2 className="text-custom-20 mobile:ml-14 ml-2 font-normal mobile:text-right">{title}</h2>
        <div className="flex items-center space-x-4 relative">
        {type === "search" ? (
  <div className="relative w-72 ml-10 flex items-center"> {/* Flex container to align items horizontally */}
    {/* Search Bar */}
    <input
      type="text"
      className="bg-gray-200 rounded-full w-full mr-12 h-10 pl-10 pr-4 text-gray-700 focus:outline-none"
      ...
      <span className="absolute top-0 right-0 bg-red-600 text-white text-xs font-bold rounded-full h-4 w-4 flex items-center justify-center">
        2
            ...
            <>
              ...
              <CustomButton className='mobile:m-8' text={buttonText} onClick={onAddStory}  />
            </>
          )}
        </div>
      </div>
