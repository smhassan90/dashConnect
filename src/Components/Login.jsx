...
const Login = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Assuming the token is in response.data.token, adjust if needed
      const token = response.data.token; // Replace with the correct path to the token if different
      if (token) {
        localStorage.setItem('your_access_token', token); // Save the token to local storage
      ...
      console.error('Error sending data:', error);
      setError("Login failed. Please try again."); // Update error message for failed login
    }
  };
