...
//     // Retrieve the token from localStorage (or sessionStorage, depending on where you store it)
...
//         {
//           apiKey: password, // Assuming password is the API key
//           userId: username, // Assuming username is the user ID
//         },
//         {
//           headers: {
...
//       setIsConnectEnabled(false); // Keep "Connect" disabled
//     }
//   };
...
//               <CustomButton
//                 text="Connect"
//                 className={`hover:text-black hover:bg-white border-2 border-black ${isConnectEnabled ? 'cursor-pointer' : 'cursor-not-allowed'}`}
//                 disabled={!isConnectEnabled} // Disable button if `isConnectEnabled` is false
//               />
//             </div>
//           </form>
//         </div>
...
function Integration() {
  ...
  //     // history.push('/login'); // Or use React Router for redirection
  //     return;
  //   }
  ...
  //     const response = await axios.post(
  //       'http://localhost:3000/api/user/testConnection',
  //       { apiKey: password, userId: username },
  //       {
  ...
  const handleTestClick = async () => {
    ...
    // Proceed with API call if token exists
    const password = "yourPassword";  // Replace with actual password value
    const username = "yourUsername";  // Replace with actual username value
    ...
    try {
      ...
      console.error('Error:', error.message);
      setIsConnectEnabled(false);  // Update UI state on error
    }
  };
